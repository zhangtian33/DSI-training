...
class DepthDecoder(nn.Module):
    def __init__(self, ch_enc = [64,128,216,288,288], scales=range(4),num_ch_enc = [ 64, 64, 128, 256, 512 ], num_output_channels=1, freeze=0):
    ...
    def forward(self, input_features):
        ...
        if self.freeze == 1:
            outputs[("disp_f", 0)] = self.sigmoid(self.convs["dispconv0"](x))
            outputs[("disp_f", 1)] = self.sigmoid(self.convs["dispconv1"](features["X_04"]))
            outputs[("disp_f", 2)] = self.sigmoid(self.convs["dispconv2"](features["X_13"]))
            outputs[("disp_f", 3)] = self.sigmoid(self.convs["dispconv3"](features["X_22"]))
        elif self.freeze == 2:
            outputs[("disp_f_0", 0)] = self.sigmoid(self.convs["dispconv0"](x))
            outputs[("disp_f_0", 1)] = self.sigmoid(self.convs["dispconv1"](features["X_04"]))
            outputs[("disp_f_0", 2)] = self.sigmoid(self.convs["dispconv2"](features["X_13"]))
            outputs[("disp_f_0", 3)] = self.sigmoid(self.convs["dispconv3"](features["X_22"]))
        else:
